...
class Block(Option):
    def validate_player(self, player, is_human=True):
        if player.ENERGY + self.ENERGY >= 0:
            ...
            if is_human:
                system.error(f"You do not have enough energy. Required: {abs(self.ENERGY)}.")
            return False
...
class Game:
    ...
    def __call__(self):
        ...
        print(system.communicate("The duel has begun.\n"
                                 "(The game may contain bugs in ACCOUNT INFO in 'action distribution - UTILITY %')", "LIGHTRED_EX"))
