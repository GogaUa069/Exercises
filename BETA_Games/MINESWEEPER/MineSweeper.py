...
class GameBoard:
    def __init__(self, n, m):
        self.N = n
        self.M = m
        self.board = [[Cell(is_mine=0, symbol="\033[0m{}".format("#")) for _ in range(self.N)] for _ in range(self.N)]
        self.row = None
        self.column = None
        self.counter = 0
    ...
    def get_indx(self):
        print()
        print("\033[0m{}".format("Enter ROW of cell."))
        self.row = int(input("-----> "))
        print()
        print("\033[0m{}".format("Enter COLUMN of cell"))
        ...
        for i in range(len(self.board)):
            for j in range(len(self.board)):
                ...
                if i+1 == self.row and j+1 == self.column:
                    ...
                    match cell.is_mine:
                        case 1:
                            cell.symbol = "\033[31m{}".format("*")
                        case 0:
                            cell.symbol = "\033[34m{}".format(cell.mines_around)

    def show(self):
        print(" ", *["\033[0m{}".format(k + 1) for k in range(len(self.board))], sep=" ")
        for row in self.board:
            print("\033[0m{}".format(self.board.index(row) + 1), end=" ")
            for cell in row:
