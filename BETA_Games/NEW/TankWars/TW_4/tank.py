...
class Tank:
    ...
    def set_effect(self, is_buff):
        print(communicate(f"\nSelect attribute to set {"buff" if is_buff else "debuff"}:", "CYAN"))
        while True:
            ...
            match attr in self.category.buffs:
                case True:
                    ...
                    break
                case False:
                    print(communicate("ERROR: The attribute name is not correct or is already in use!\n", "LIGHTRED_EX"))
    ...
    def __str__(self):
        return communicate(f"\nUNIT INFO:\n"
                f"- id:       {self.__id}\n"
                f"- rank:     {self.rank}\n"
                f"- nation:   {self.nation.name} ({colors[self.nation.color]})\n"
