...
class System:
    ...
    def print_ascii_text(self, ascii_text, color="LIGHTGREEN_EX"):
        for line in ascii_text.splitlines():
            print(self.communicate(line, color))
            time.sleep(0.25)

    @staticmethod
    def get_ascii_text(text, color="LIGHTGREEN_EX", font="standard"):
        f = pyfiglet.Figlet(font=font)
        ascii_text = f.renderText(text)
        return system.communicate(ascii_text, color)
...
class Intro:
    ...
    def show_game_banner(*banner):
        ...
        time.sleep(1.5)
        play_soundtrack(r"audio/sounds/short_shoot_sound.mp3")
        print(system.get_ascii_text(banner[0]), end="")
        time.sleep(1.5)
        play_soundtrack(r"audio/sounds/lmg_reloading_sound.mp3")
        print(system.get_ascii_text(banner[1]))
        time.sleep(1.5)
